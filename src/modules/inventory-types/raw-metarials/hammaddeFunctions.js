...
function hesaplaHammaddeGirisiToleransli() {
  console.log("hesaplaHammaddeGirisiToleransli çağrıldı");

  const hammaddeTuru = document.getElementById('hammaddeGirisTuruDegeri').value;
  const sonucAlani = document.getElementById('hammaddeGirisHesaplamaSonucu');
  
  // Miktar değerini kontrol et
  const miktarInput = document.getElementById('hammaddeGirisMiktar');
  const miktarDegeri = miktarInput.value.trim();
  
  // Miktar boşsa veya sayı değilse
  if (!miktarDegeri) {
      sonucAlani.innerHTML = `<div class="error">Lütfen bir miktar girin.</div>`;
      return;
  }
  
  const miktar = parseFloat(miktarDegeri);
  if (isNaN(miktar) || miktar <= 0) {
      sonucAlani.innerHTML = `<div class="error">Lütfen geçerli bir miktar girin.</div>`;
      return;
  }
  
  // Kaydet butonunu önce devre dışı bırak
  const kaydetBtn = document.getElementById('hammaddeGirisKaydetBtn');
  if (kaydetBtn) {
      kaydetBtn.disabled = false; // Daima aktif olsun, hesaplamayla ilişkisi kalmadı
  }
  
  try {
      if (hammaddeTuru === 'sac') {
          // Saç hesaplaması - değişiklik yok, olduğu gibi kalacak
          const kalinlik = parseFloat(document.getElementById('sacKalinlik').value) / 1000; // mm → m
          const en = parseFloat(document.getElementById('sacEn').value) / 1000; // mm → m
          const boy = parseFloat(document.getElementById('sacBoy').value) / 1000; // mm → m
          const yogunluk = parseFloat(document.getElementById('sacYogunluk').value);
          
          // Tek plaka ağırlığı hesapla
          const hacim = en * boy * kalinlik;
          const plakaAgirligi = hacim * yogunluk;
          
          if (plakaAgirligi <= 0) {
              sonucAlani.innerHTML = `<div class="error">Geçersiz plaka ağırlığı hesaplandı.</div>`;
              return;
          }
          
          // İdeal plaka sayısı hesapla (virgüllü olabilir)
          const idealPlakaSayisi = miktar / plakaAgirligi;
          ...
          // Alt ve üst plaka sayılarına göre beklenen ağırlıklar
          const altToplamAgirlik = altPlakaSayisi * plakaAgirligi;
          const ustToplamAgirlik = ustPlakaSayisi * plakaAgirligi;
          
          // Toplam ağırlık için tolerans miktarlarını hesapla
          const toleransYuzdesi = 1; // %1 tolerans
          const altTolerانsMiktari = altToplamAgirlik * (toleransYuzdesi / 100);
          const ustToleransMiktari = ustToplamAgirlik * (toleransYuzdesi / 100);
          
          // Alt ve üst tolerans aralıkları
          const altMinAgirlik = altToplamAgirlik - altTolerانsMiktari;
          const altMaxAgirlik = altToplamAgirlik + altTolerانsMiktari;
          ...
          let gercekPlakaAgirligi;
          
          // 1) Miktar, alt plaka sayısının tolerans aralığında mı?
          if (miktar >= altMinAgirlik && miktar <= altMaxAgirlik) {
              plakaSayisi = altPlakaSayisi;
              gercekPlakaAgirligi = miktar / plakaSayisi;
              toleransUyarisi = `<div class="success-text">Girilen miktar ${plakaSayisi} plaka için tolerans (±%${toleransYuzdesi}) içindedir.</div>`;
              toleransliMi = true;
              if (kaydetBtn) kaydetBtn.disabled = false;
          }
          // 2) Miktar, üst plaka sayısının tolerans aralığında mı?
          else if (miktar >= ustMinAgirlik && miktar <= ustMaxAgirlik) {
          ...
          // 3) Hiçbir tolerans aralığına uymuyorsa
          else {
              // Girilen miktar hangi tam plaka sayısına daha yakın
              const altFark = Math.abs(miktar - altToplamAgirlik);
              const ustFark = Math.abs(miktar - ustToplamAgirlik);
              ...
              const beklenenAgirlik = plakaSayisi * plakaAgirligi;
              
              toleransUyarisi = `
                  <div class="error-text">
                      Girilen miktar (${miktar.toFixed(2)} kg) ${plakaSayisi} plaka için tolerans (±%${toleransYuzdesi}) dışındadır.
                      ...
                      <br>- veya tam olarak: ${altToplamAgirlik.toFixed(2)} kg (${altPlakaSayisi} plaka) ya da ${ustToplamAgirlik.toFixed(2)} kg (${ustPlakaSayisi} plaka)
                  </div>
              `;
              
              gercekPlakaAgirligi = beklenenAgirlik / plakaSayisi;
              toleransliMi = false;
          ...
          sonucAlani.innerHTML = `
              <div class="calculation-details">
                  <div class="detail-row">
                      <span>Miktar:</span>
                      <span>${miktar.toFixed(2)} kg</span>
                  </div>
                  <div class="detail-row">
                      <span>Bir plaka ağırlığı:</span>
                      <span>${plakaAgirligi.toFixed(2)} kg</span>
                  </div>
                  <div class="detail-row">
                      <span>Gerçek plaka ağırlığı:</span>
                      <span>${gercekPlakaAgirligi.toFixed(2)} kg</span>
                  </div>
                  <div class="detail-row">
                      <span>Oluşturulacak plaka sayısı:</span>
                      <span>${plakaSayisi}</span>
                  </div>
                  <div class="detail-row">
                      <span>Tolerans:</span>
                      <span>±%${toleransYuzdesi} (${plakaSayisi} plaka için: ${(plakaSayisi * plakaAgirligi * (toleransYuzdesi / 100)).toFixed(2)} kg)</span>
                  </div>
                  ${toleransUyarisi}
              </div>
          `;
          ...
          window.hesaplamaDetaylari = {
              plakaAgirligi: plakaAgirligi,
              gercekPlakaAgirligi: gercekPlakaAgirligi,
          ...
          // Boru ve mil için basitleştirilmiş hesaplama - burada değişiklik yapıyoruz
          // Sadece basit bilgi görüntüle, herhangi bir adet hesaplaması yapma
          
          // 1 parça olarak kaydet
          const plakaSayisi = 1;
          const plakaAgirligi = miktar;
          ...
          sonucAlani.innerHTML = `
              <div class="calculation-details">
                  ...
                  <div class="success-text">Toplam ${miktar.toFixed(2)} kg ağırlığında 1 parça oluşturulacak</div>
              </div>
          `;
          
          // Hesaplama sonucunu global değişkende sakla
          window.hesaplamaDetaylari = {
              plakaAgirligi: miktar,
              gercekPlakaAgirligi: miktar,
              plakaSayisi: 1,
              toleransliMi: true,
              miktar: miktar
          };
      ...
      console.error('Hammadde girişi hesaplama hatası:', error);
      sonucAlani.innerHTML = `<div class="error">Hesaplama yapılırken bir hata oluştu.</div>`;
      
      // Hata durumunda kaydet butonu devre dışı
      if (kaydetBtn) kaydetBtn.disabled = true;
  }
}
...
function calculateParts() {
    // Form değerlerini al
    const en = parseFloat(document.getElementById('en').value) / 1000; // mm'den m'ye çevir
    const boy = parseFloat(document.getElementById('boy').value) / 1000; // mm'den m'ye çevir
    const kalinlik = parseFloat(document.getElementById('kalinlik').value) / 1000; // mm'den m'ye çevir
    const toplamKilo = parseFloat(document.getElementById('toplamKilo').value);
    ...
    // Plaka sayısını hesapla
    const plakaSayisi = Math.floor(toplamKilo / plakaAgirligi);
    
    // Hesaplama detaylarını güncelle
    document.getElementById('hacim').textContent = hacim.toFixed(6) + ' m³';
    document.getElementById('plakaAgirligi').textContent = plakaAgirligi.toFixed(2) + ' kg';
    document.getElementById('plakaSayisi').textContent = plakaSayisi;
    
    // Hesaplama detaylarını göster
    document.getElementById('hesaplamaDetaylari').classList.remove('hidden');
    
    // Parçaları oluştur
    generateParts(plakaSayisi, plakaAgirligi);
    
    // Kaydet butonunu etkinleştir
    document.getElementById('kaydetBtn').disabled = false;
    
    // Daha sonra kullanmak için değerleri sakla
    parcaSayisi = plakaSayisi;
    currentPartsData = {
...
// Parça kartlarını oluşturma
function generateParts(count, weight) {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        `;
        parcaListesi.appendChild(parcaCard);
    }
}
...
async function viewHammaddeDetail(id) {
    try {
      ...
      <div class="detay-value">${(() => {
        ...
        // Load history data
        await loadIslemGecmisi(id);
        await loadHammaddeGirisGecmisi(id);
...
function hesaplaHammaddeGirisi() {
    const hammaddeTuru = document.getElementById('hammaddeGirisTuruDegeri').value;
    const sonucAlani = document.getElementById('hammaddeGirisHesaplamaSonucu');
    ...
    try {
        if (hammaddeTuru === 'sac') {
            // Saç hesaplaması
            const miktar = parseFloat(document.getElementById('hammaddeGirisMiktar').value) || 0;
            const kalinlik = parseFloat(document.getElementById('sacKalinlik').value) / 1000; // mm → m
            const en = parseFloat(document.getElementById('sacEn').value) / 1000; // mm → m
            const boy = parseFloat(document.getElementById('sacBoy').value) / 1000; // mm → m
            const yogunluk = parseFloat(document.getElementById('sacYogunluk').value);
            ...
            // Plaka sayısını hesapla
            const plakaSayisi = Math.floor(miktar / plakaAgirligi);
            ...
            // Boru hesaplaması
            const adet = parseInt(document.getElementById('hammaddeGirisMiktar').value) || 0;
            const cap = parseFloat(document.getElementById('boruCap').value);
            const kalinlik = parseFloat(document.getElementById('boruKalinlik').value);
            const uzunluk = parseFloat(document.getElementById('boruUzunluk').value);
            const yogunluk = parseFloat(document.getElementById('boruYogunluk').value);
            
            // Boru ağırlığını hesapla
            const boruAgirligi = calculateBoruWeight({ cap, kalinlik, uzunluk, yogunluk });
            const toplamAgirlik = boruAgirligi * adet;
            
            // Sonucu göster - adet, uzunluk ve ağırlık bilgilerini detaylı göster
            sonucAlani.innerHTML = `
                <div class="calculation-details">
                    <div class="detail-row">
                        <span>Adet:</span>
                    ...
                    <div class="detail-row">
                        <span>Boyut:</span>
                        <span>Ø${cap}x${kalinlik} mm, Boy: ${uzunluk} mm</span>
                    </div>
                    <div class="detail-row">
                        <span>Bir boru ağırlığı:</span>
                        <span>${boruAgirligi.toFixed(2)} kg</span>
                    </div>
                    <div class="detail-row">
                        <span>Toplam ağırlık:</span>
            ...
            // Mil hesaplaması
            const adet = parseInt(document.getElementById('hammaddeGirisMiktar').value) || 0;
            const cap = parseFloat(document.getElementById('milCap').value);
            const uzunluk = parseFloat(document.getElementById('milUzunluk').value);
            const yogunluk = parseFloat(document.getElementById('milYogunluk').value);
            
            // Mil ağırlığını hesapla
            const milAgirligi = calculateMilWeight({ cap, uzunluk, yogunluk });
            const toplamAgirlik = milAgirligi * adet;
            ...
            sonucAlani.innerHTML = `
                <div class="calculation-details">
                    ...
                    <div class="detail-row">
                        <span>Boyut:</span>
                        <span>Ø${cap} mm, Boy: ${uzunluk} mm</span>
                    </div>
                    <div class="detail-row">
                        <span>Bir mil ağırlığı:</span>
                        <span>${milAgirligi.toFixed(2)} kg</span>
                    </div>
                    <div class="detail-row">
                        <span>Toplam ağırlık:</span>
                        <span>${toplamAgirlik.toFixed(2)} kg</span>
...
async function openHammaddeGirisGuncelleModal(girisId, hammaddeId, mevcutMiktar) {
  try {
    ...
    currentHammaddeGirisId = girisId;
    currentHammaddeId = hammaddeId;
    
    // Modal başlığını güncelle
    document.getElementById('hammaddeGirisGuncelleHeader').textContent = 'Hammadde Giriş Güncelleme';
    
    // Hammadde bilgilerini al
    const result = await window.electronAPI.invoke.database.getHammaddeById(hammaddeId);
    
    if (!result.success) {
      showToast('Hammadde bilgileri alınamadı', 'error');
      return;
    }
    
    // Giriş detaylarını al
    const girisDetaylari = await window.electronAPI.invoke.database.getHammaddeGirisById(girisId);
    if (girisDetaylari.success && girisDetaylari.giris) {
      ...
      const mevcutMiktarElem = document.getElementById('hammaddeGunceleMevcutMiktar');
      const birimLabel = document.querySelector('label[for="hammaddeGirisGuncelleMiktar"]');
      
      if (mevcutMiktarElem) {
        mevcutMiktarElem.textContent = hammaddeTuru === 'sac' ? 
          `${Number(mevcutMiktar).toFixed(2)} kg` : 
          `${Number(mevcutMiktar).toFixed(2)} kg`;
      }
      
      if (birimLabel) {
        birimLabel.textContent = 'Yeni Miktar (kg)';
      }
      
      // Form alanlarını mevcut değerlerle doldur
      document.getElementById('hammaddeGirisGuncelleMiktar').value = mevcutMiktar;
      
      // Birim fiyat
      if (document.getElementById('hammaddeGirisGuncelleBirimFiyat')) {
        document.getElementById('hammaddeGirisGuncelleBirimFiyat').value = giris.birim_fiyat || '';
      }
      
      // Para birimi seçimi
      if (document.getElementById('hammaddeGirisGuncelleBirimFiyatTuru')) {
        document.getElementById('hammaddeGirisGuncelleBirimFiyatTuru').value = giris.birim_fiyat_turu || 'TRY';
      }
      
      // Ana barkod değerini göster
      if (document.getElementById('hammaddeGirisGuncelleAnaBarkod')) {
        document.getElementById('hammaddeGirisGuncelleAnaBarkod').value = giris.ana_barkod || '';
      }
      
      // Tedarikçi
      if (document.getElementById('hammaddeGirisGuncelleTedarikci')) {
        document.getElementById('hammaddeGirisGuncelleTedarikci').value = giris.tedarikci || '';
      }
      
      // Kritik seviye
      document.getElementById('hammaddeGirisGuncelleKritikSeviye').value = hammadde.kritik_seviye || 0;
    }
    ...
    setTimeout(() => {
      document.getElementById('hammaddeGirisGuncelleMiktar').focus();
    }, 300);
  } catch (error) {
    console.error('Hammadde giriş güncelleme modalı açma hatası:', error);
    showToast('Modal açılırken bir hata oluştu', 'error');
  }
}


async function guncelleHammaddeGirisi() {
  try {
    // Form değerlerini al
    const girisId = currentHammaddeGirisId;
    const hammaddeId = currentHammaddeId;
    const yeniMiktar = parseFloat(document.getElementById('hammaddeGirisGuncelleMiktar').value);
    const birimFiyat = parseFloat(document.getElementById('hammaddeGirisGuncelleBirimFiyat').value || 0);
    ...
    const kritikSeviye = parseFloat(document.getElementById('hammaddeGirisGuncelleKritikSeviye').value || 0);
    const anaBarkod = document.getElementById('hammaddeGirisGuncelleAnaBarkod').value.trim();
    const ekleyenId = document.getElementById('hammaddeGirisGuncelleEkleyenId')?.value || window.currentUserId;
    ...
    // Ana barkod zorunlu
    if (!anaBarkod) {
      showModalError('hammaddeGirisGuncelleModal', 'Lütfen ana barkod girin.');
      return;
    }
    ...
    if (!hammaddeResult.success) {
      showErrorMessage('Hata', 'Hammadde bilgileri alınamadı: ' + hammaddeResult.message);
      return;
    }
    ...
    // İşlem kaydediliyor mesajını göster
    showModalSuccess('hammaddeGirisGuncelleModal', 'Güncelleme yapılıyor...');
    ...
    const guncelleData = {
      ...
      tedarikci: tedarikci,
      kritik_seviye: kritikSeviye,
      ana_barkod: anaBarkod, // Ana barkod değeri eklendi
    ...
    showErrorMessage('İşlem Hatası', 'Hammadde girişi güncellenirken bir hata oluştu: ' + error.message);
  }
}

// Miktar değiştiğinde hesaplama yapılsın
function initHammaddeGirisGuncelleEvents() {
    const miktarInput = document.getElementById('hammaddeGirisGuncelleMiktar');
    if (miktarInput) {
        miktarInput.addEventListener('input', function() {
            // Hesaplama sonuçlarını göstermek için fonksiyonu çağır
            // Ama veritabanına kaydetme
            hesaplaHammaddeGirisiGuncelle();
        });
    }
}

// Sadece hesaplama yapan fonksiyon
async function hesaplaHammaddeGirisiGuncelle() {
    const hammaddeId = currentHammaddeId;
    const yeniMiktar = parseFloat(document.getElementById('hammaddeGirisGuncelleMiktar').value);
    
    // Sonuç alanını tanımla
    const sonucAlani = document.getElementById('hammaddeGirisGuncelleSonuc') || 
        document.querySelector('.modal-body .result-area') || 
        document.createElement('div');
    
    // Kaydet butonunu tanımla
    const kaydetBtn = document.getElementById('hammaddeGirisGuncelleKaydetBtn');
    if (kaydetBtn) {
        kaydetBtn.disabled = true; // Başlangıçta inaktif
    }
    
    // Miktar değerini kontrol et
    if (!yeniMiktar || isNaN(yeniMiktar) || yeniMiktar <= 0) {
        sonucAlani.innerHTML = `<div class="error">Lütfen geçerli bir miktar girin.</div>`;
        return;
    }
    ...
    try {
        ...
        if (!hammaddeResult.success) {
            sonucAlani.innerHTML = `<div class="error">Hammadde bilgileri alınamadı.</div>`;
            return;
        }
        ...
        // Hammadde türüne göre hesaplama
        if (hammaddeTuru === 'sac') {
            const toleransYuzdesi = 1; // %1 tolerans
            const kalinlik = parseFloat(hammadde.kalinlik) / 1000; // mm → m
            const en = parseFloat(hammadde.en) / 1000; // mm → m
            const boy = parseFloat(hammadde.boy) / 1000; // mm → m
            const yogunluk = parseFloat(hammadde.yogunluk);
            ...
            // İdeal plaka sayısı hesapla (virgüllü olabilir)
            const idealPlakaSayisi = yeniMiktar / plakaAgirligi;
            ...
            // Toplam ağırlık için tolerans miktarlarını hesapla
            const altToleransMiktari = altToplamAgirlik * (toleransYuzdesi / 100);
            const ustToleransMiktari = ustToplamAgirlik * (toleransYuzdesi / 100);
            ...
            // 1) Miktar, alt plaka sayısının tolerans aralığında mı?
            if (yeniMiktar >= altMinAgirlik && yeniMiktar <= altMaxAgirlik) {
                plakaSayisi = altPlakaSayisi;
                gercekPlakaAgirligi = yeniMiktar / plakaSayisi;
                toleransUyarisi = `<div class="success-text">Girilen miktar ${plakaSayisi} plaka için tolerans (±%${toleransYuzdesi}) içindedir.</div>`;
                toleransliMi = true;
                if (kaydetBtn) kaydetBtn.disabled = false;
            }
            // 2) Miktar, üst plaka sayısının tolerans aralığında mı?
            else if (yeniMiktar >= ustMinAgirlik && yeniMiktar <= ustMaxAgirlik) {
                plakaSayisi = ustPlakaSayisi;
                gercekPlakaAgirligi = yeniMiktar / plakaSayisi;
                toleransUyarisi = `<div class="success-text">Girilen miktar ${plakaSayisi} plaka için tolerans (±%${toleransYuzdesi}) içindedir.</div>`;
                toleransliMi = true;
            ...
            // 3) Hiçbir tolerans aralığına uymuyorsa
                ...
                // Girilen miktar hangi tam plaka sayısına daha yakın
                const altFark = Math.abs(yeniMiktar - altToplamAgirlik);
                const ustFark = Math.abs(yeniMiktar - ustToplamAgirlik);
                ...
                toleransUyarisi = `
                    <div class="error-text">
                        Girilen miktar (${yeniMiktar.toFixed(2)} kg) ${plakaSayisi} plaka için tolerans (±%${toleransYuzdesi}) dışındadır.
                        <br>Lütfen şu değerlerden birini girin:
                        <br>- ${altPlakaSayisi} plaka için: ${altMinAgirlik.toFixed(2)} kg - ${altMaxAgirlik.toFixed(2)} kg
                        <br>- ${ustPlakaSayisi} plaka için: ${ustMinAgirlik.toFixed(2)} kg - ${ustMaxAgirlik.toFixed(2)} kg
                        <br>- veya tam olarak: ${altToplamAgirlik.toFixed(2)} kg (${altPlakaSayisi} plaka) ya da ${ustToplamAgirlik.toFixed(2)} kg (${ustPlakaSayisi} plaka)
            ...
            sonucAlani.innerHTML = `
                <div class="calculation-details">
                    <div class="detail-row">
                        <span>Miktar:</span>
                        <span>${yeniMiktar.toFixed(2)} kg</span>
                    </div>
                    <div class="detail-row">
                        <span>Bir plaka ağırlığı:</span>
                        <span>${plakaAgirligi.toFixed(2)} kg</span>
            ...
            // Boru ve mil için birim ağırlık hesaplaması
            const birimAgirlik = hammaddeTuru === 'boru' 
                ? calculateBoruWeight(hammadde) 
                : calculateMilWeight(hammadde);
            
            const toplamAgirlik = birimAgirlik * yeniMiktar;
            ...
            sonucAlani.innerHTML = `
                <div class="calculation-details">
                    <div class="detail-row">
                        <span>Adet:</span>
                        <span>${yeniMiktar}</span>
                    </div>
                    <div class="detail-row">
                        <span>Bir ${hammaddeTuru} ağırlığı:</span>
                        <span>${birimAgirlik.toFixed(2)} kg</span>
                    </div>
                    <div class="detail-row">
                        <span>Toplam ağırlık:</span>
                        <span>${toplamAgirlik.toFixed(2)} kg</span>
                    </div>
                    <div class="success-text">Değerler geçerli, kaydedebilirsiniz.</div>
                </div>
            `;
        ...
        console.error('Hammadde girişi hesaplama hatası:', error);
        sonucAlani.innerHTML = `<div class="error">Hesaplama yapılırken bir hata oluştu: ${error.message}</div>`;
    }
}
  ...
  // Event listener'lar
document.addEventListener('DOMContentLoaded', function() {
    // Hammadde giriş güncelle kaydet butonu
    const hammaddeGirisGuncelleKaydetBtn = document.getElementById('hammaddeGirisGuncelleKaydetBtn');
    if (hammaddeGirisGuncelleKaydetBtn) {
        hammaddeGirisGuncelleKaydetBtn.addEventListener('click', guncelleHammaddeGirisi);
    }
});
